	...
	return (
		<div>
			<div className="card-container">
				{data.map((rental) => (
					<div key={rental.id} className="card" onClick={() => handleCardClick(rental.id)}>
						<img src={rental.image1} alt={rental.location} />
						<div className={`like ${likedCards.includes(rental.id) ? "liked" : ""}`} onClick={() => handleClick(rental.id)}>
							{likedCards.includes(rental.id) ? <IoMdHeart className="like" /> : <IoMdHeartEmpty className="like" />}
						</div>
