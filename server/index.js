...
app.post("/api/signup", async (req, res) => {
	try {
		...
		const isValid = await db.query("SELECT * FROM users WHERE username = $1", [username]);

		if (isValid.rows[0]) {
			throw new Error("User already exists");
		}

		const userData = await db.query("INSERT INTO users(username, password) VALUES ($1, $2) RETURNING *", [username, password]);
